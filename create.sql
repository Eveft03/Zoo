...
CREATE TABLE
    TICKET (
        ...
        CATEGORY ENUM ('Event', 'No evnt') NOT NULL,
        PRIMARY KEY (CODE, DATE_OF_ISSUE),
        FOREIGN KEY (CASHIER_ID) REFERENCES CASHIER (ID),
        FOREIGN KEY (Email) REFERENCES VISITOR (Email),
        CONSTRAINT CHK_IF_SUNDAY CHECK (DAYOFWEEK (DATE_OF_ISSUE) NOT IN (1))
    );
...
CREATE TABLE
    REQUIRES (
        ...
        FOREIGN KEY (TITLE, EVENT_DATE) REFERENCES EKDILOSI (TITLE, EVENT_DATE),
        FOREIGN KEY (CODE, DATE_OF_ISSUE) REFERENCES EISITIRIO (CODE, DATE_OF_ISSUE)
    );
