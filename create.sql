...
CREATE TRIGGER EVENT_CHECK BEFORE INSERT ON TICKET
FOR EACH ROW 
BEGIN
    ...
    AND NOT EXISTS (
        ...
        FROM EVENT
        WHERE EVENT.EVENT_DATE = NEW.DATE_OF_ISSUE
    ) THEN 
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No event on this date';
    END IF;
END $$

CREATE TRIGGER EVENT_COUNT BEFORE INSERT ON EVENT
FOR EACH ROW 
BEGIN
    IF (
        SELECT COUNT(*)
        FROM EVENT
        WHERE NEW.EVENT_DATE = EVENT_DATE
    ) >= 2 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '2 events already scheduled for this date';
    END IF;
END $$

CREATE TRIGGER TICKET_CHECK BEFORE INSERT ON REQUIRES
FOR EACH ROW 
BEGIN
    IF (
        SELECT TICKET.CATEGORY
        FROM TICKET
        WHERE TICKET.CODE = NEW.CODE
    ) = 'No event' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'This ticket can not be used for an event';
    ELSEIF NEW.EVENT_DATE != NEW.DATE_OF_ISSUE THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'This ticket is not valid for the event';
    END IF;
END $$
